...
filename = r'C:\Users\rnili\OneDrive\Desktop\VCA-stable\test.yuv'
width = 1920
height = 1080

# Calculate the size of the first frame in bytes (YUV420 planar format)
frame_size = int(1.5 * width * height)

with open(filename, 'rb') as file:
    frame_data = file.read(frame_size)

# Convert the binary frame data to NumPy arrays for Y, U, V components
Y = np.frombuffer(frame_data[:width * height], dtype=np.uint8).reshape(height, width)
print("Y Length:", len(Y))
block = Y[0:32, 0:32].flatten()
...
start = timer()
buffer = partial_butterfly32(block, 4, 32)
print('========================== CALL END ==========================')
DCT = partial_butterfly32(buffer, 11, 32)
print("DCT:", DCT)
avg = sum(DCT) / len(DCT)
print("Average:", avg)
print("Time no-GPU:", timer()-start)